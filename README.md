...
![子项目概览](./assets/readme/projects-overview.svg)
...
![学习路径](./assets/readme/learning-path.svg)

五个阶段从 Vue 3 工程基础出发,依次经过组件组合、路由跳转、状态管理,最终回到三个机制的综合应用。除"基础"阶段对应三个子项目共享的工程模板外,其余每个阶段都对应仓库里的一个真实子项目。
...
![技术栈分层](./assets/readme/tech-stack.svg)

四层都是仓库内真实在用的技术,没有列入未使用的库。Vue 3 与 Composition API 提供响应式基础,Vite 负责开发服务器与构建,Vue Router 处理路由,Vuex 集中管理状态。

---

## 组件模式示意

![组件模式](./assets/readme/component-patterns.svg)

Vue 3 组件间通信常见四种模式。本仓库的 `slot-practice` 覆盖了其中的 **Slot** 模式(默认插槽 / 具名插槽 / 作用域插槽);Props、Emit、Composable 是 Vue 3 通用组件模式,作为后续学习的延伸方向,不在当前三个练习的范围内。

---

## 如何运行

![命令行用法](./assets/readme/terminal-usage.svg)
...
![footer](./assets/readme/footer.svg)
